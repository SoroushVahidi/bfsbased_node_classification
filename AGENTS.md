...
- The `prl_resubmission_runner.py` imports `manuscript_runner` and `standard_node_baselines` as sibling modules. It must be invoked from the repo root or with `code/bfsbased_node_classification` on `sys.path`.
- GCN/APPNP baseline training (via `prl_resubmission_runner.py`) takes ~50s per dataset-split on CPU due to grid search. MLP + SGC alone is much faster (~2s).
