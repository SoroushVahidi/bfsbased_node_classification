...
out_root = Path('/workspace/logs/prl_local_20260319/exp02_threshold_sensitivity')
split_dir = '/workspace/data/geom-gcn-splits-broad'
datasets = ['cora', 'citeseer']
splits = list(range(10))
repeats = [0]
threshold_grid = [0.03, 0.05, 0.08, 0.12, 0.18, 0.25, 0.35, 0.50, 0.65]
...
raw_path = out_root / 'raw' / 'threshold_sweep_runs.jsonl'
...
summary_csv = out_root / 'tables' / 'threshold_sweep_summary.csv'
...
uc_csv = out_root / 'tables' / 'threshold_vs_uncertain_changed.csv'
...
for ds in datasets:
    ...
    ax.set_title(f'Exp02 threshold sensitivity: {ds}')
    fig.tight_layout()
    fig.savefig(out_root / 'plots' / f'threshold_gain_curve_{ds}.png', dpi=220)
    plt.close(fig)

notes = out_root / 'notes' / 'threshold_sensitivity_notes.md'
lines = [
...
for ds in datasets:
    ...
    lines.append(f"- {ds}: best threshold={best['threshold']:.2f}, mean_delta_vs_mlp={best['mean_delta_vs_mlp']:+.4f}, avg_uncertain={best['avg_uncertain']:.4f}, avg_changed={best['avg_changed']:.4f}")
notes.write_text('\n'.join(lines) + '\n', encoding='utf-8')

print(raw_path)
print(summary_csv)
print(uc_csv)
print(out_root / 'plots' / 'threshold_gain_curve_cora.png')
print(out_root / 'plots' / 'threshold_gain_curve_citeseer.png')
print(notes)
PY
