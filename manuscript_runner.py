        ...
        for split_id in avail_splits:
            for repeat in range(repeats):
                ...
                if "selective_graph_correction" in methods and mlp_probs is not None:
                    ...
                    t0 = time.perf_counter()
                    try:
                        _, acc_sgc, diag = mod.selective_graph_correction_predictclass(
                            data,
                            train_np,
                            test_np,
                            mlp_probs=mlp_probs,
                            val_indices=val_idx,
                            tune_on_val=True,
                            tune_n_weight_samples=sgc_tune_n_samples,
                            seed=current_seed,
                            log_file=None,
                        ...
                        # Quick val acc for completeness (re-run without test leakage)
                        _, acc_val_sgc, _ = mod.selective_graph_correction_predictclass(
                            data,
                            train_np,
                            val_np,
                            mlp_probs=mlp_probs,
                            val_indices=None,
                            tune_on_val=False,
                            b1=diag["b1"], b2=diag["b2"],
                            b3=diag["b3"], b4=diag["b4"], b5=diag["b5"],
                            threshold_high=diag["threshold_high"],
                            seed=current_seed,
                            log_file=None,
                            log_prefix=f"sgc_val_{dataset_key}_s{split_id}_r{repeat}",
                            record_diagnostics=False,
                        )
                        rec.update({
                            "val_acc": float(acc_val_sgc),
                            "test_acc": float(acc_sgc),
                            "tuning_runtime_sec": 0.0,
                            "method_runtime_sec": method_time,
                            "total_runtime_sec": method_time,
                            "parameter_source": "val_tuned_threshold_and_b_weights",
                            "sgc_threshold": diag["threshold_high"],
                            "sgc_b_weights": [diag["b1"], diag["b2"], diag["b3"], diag["b4"], diag["b5"]],
                            "sgc_n_uncertain": diag["n_uncertain"],
                            "sgc_n_changed": diag["n_changed"],
                        })
                    except Exception as e:
                        rec["success"] = False
                        rec["notes"] = str(e)
...
def generate_case_studies(
    ...
    for rec in sgc_records:
        ...
        diag_path = os.path.join("logs", f"sgc_{run_id}_diagnostics.json")
        if not os.path.isfile(diag_path):
            continue
        with open(diag_path, encoding="utf-8") as df:
            diag = json.load(df)
        node_diags = diag.get("node_diagnostics", [])
        changed = [nd for nd in node_diags if nd.get("changed", False)]

        lines.append(f"## {ds}\n\n")
        lines.append(
            f"- threshold_high={diag['threshold_high']:.2f}  "
            f"b=({diag['b1']:.2f},{diag['b2']:.2f},{diag['b3']:.2f},{diag['b4']:.2f})\n"
        )
        lines.append(
            f"- {diag['n_uncertain']}/{diag['n_test']} test nodes uncertain  "
            f"| {diag['n_changed']} corrected\n\n"
        )

        if changed:
            lines.append("### Sample corrected nodes (first 10)\n\n")
            lines.append(
                "| node_id | mlp_pred | corrected | true | mlp_ok | sgc_ok | "
                "margin | dom_component |\n"
            )
            lines.append("|---------|----------|-----------|------|--------|--------|--------|---------------|\n")
            for nd in changed[:10]:
                ...
                lines.append(
                    ...
                    f"{nd['true_label']} | {nd['mlp_correct']} | {nd['corrected_correct']} | "
