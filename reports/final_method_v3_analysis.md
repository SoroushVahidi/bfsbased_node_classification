...
- **Source log:** `reports/final_method_v3_results.csv` (rebuilt from the frozen 10-split export under `reports/archive/superseded_final_tables_prl/` via `scripts/build_artifacts/rebuild_final_v3_results_10split.py`)
...
**Chameleon (split 4):** FINAL_V3 is slightly below MLP (−0.44 pp). Ten-split means remain within noise of the MLP (see `tables/main_results_selective_correction.md`).
...
2. **Occasional split-level regressions:** e.g. Chameleon split 4 and Texas split 5 show negative Δ vs MLP for FINAL_V3; ten-split means still track the MLP on Chameleon and improve Texas on average (see `tables/main_results_selective_correction.md`)
...
Generated by `python3 scripts/build_artifacts/build_prl_auxiliary_tables.py` (also invoked from `bash scripts/run_all_selective_correction_results.sh`):

- `tables/experimental_setup_selective_correction.md` — six benchmark datasets, sizes, edge homophily, 10-split protocol, compared methods
- `tables/ablation_selective_correction.md` — **FINAL_V3** row from the 10-split log; other rows from archived Table 7 (historical, not re-run)
- `tables/sensitivity_selective_correction.md` — archived ρ-sweep snapshot (3-split, illustrative)
...
- `bash scripts/run_all_selective_correction_results.sh` — see `scripts/README_REPRODUCE.md`
