"""
GPRGNN runner - Adaptive Universal Generalized PageRank GNN
Inline reimplementation. Source: https://github.com/jianhao2016/GPRGNN
"""
import sys
import os
import argparse
...
SOURCE_REPO = "https://github.com/jianhao2016/GPRGNN"
METHOD = "gprgnn"


def get_norm_adj(edge_index, n_nodes, add_self_loop=True):
...
class GPRGNN(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, K=10, alpha=0.1, dropout=0.5):
        ...
        # GPR weights initialized from PPR
        gamma_init = [alpha * (1 - alpha) ** k for k in range(K + 1)]
        s = sum(gamma_init)
        gamma_init = [g / s for g in gamma_init]
        self.gamma = nn.Parameter(torch.tensor(gamma_init, dtype=torch.float))
...
def run(args):
    ...
    if config_path is None:
        config_path = os.path.join(_BENCH_DIR, 'configs', 'gprgnn.yaml')
    ...
    model = GPRGNN(n_features, hidden, n_classes, K=K, alpha=alpha, dropout=dropout)
    ...
    print(
        f"[{METHOD}] {args.dataset} split={args.split_id} seed={args.seed} "
        f"val={best_val_acc:.4f} test={best_test_acc:.4f} "
        f"({runtime:.1f}s) -> {out_file}"
    )
    return result


def main():
    parser = argparse.ArgumentParser(description='Run GPRGNN benchmark')
