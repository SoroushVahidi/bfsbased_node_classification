#!/usr/bin/env python3
"""Standalone PairNorm baseline runner (DeepGCN + PairNorm)."""
...
def run_pairnorm(
    ...
    output_tag: str = "pairnorm_external_baseline",
) -> Dict[str, Any]:
    if datasets is None:
    ...
    if not runnable_datasets:
        raise SystemExit("No runnable datasets found for PairNorm baseline.")

    runs_path = os.path.join(LOGS_DIR, f"pairnorm_runs_{output_tag}.jsonl")
    csv_path = os.path.join(REPORTS_DIR, f"pairnorm_results_{output_tag}.csv")
    ...
    print(f"\nCompleted {len(all_records)} PairNorm runs in {wall_time:.1f}s")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run standalone DeepGCN+PairNorm external baseline.")
    parser.add_argument("--split-dir", default=None)
    parser.add_argument("--datasets", nargs="+", default=None)
    parser.add_argument("--splits", nargs="+", type=int, default=list(range(10)))
    parser.add_argument("--output-tag", default="pairnorm_external_baseline")
