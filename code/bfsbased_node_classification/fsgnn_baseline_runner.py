#!/usr/bin/env python3
"""Standalone FSGNN external baseline runner.

This evaluates a compact in-repo adaptation of FSGNN (Feature Selection Graph
Neural Network; Maurya et al.) using this repository's canonical split protocol.
"""
...
def run_fsgnn(
    ...
    output_tag: str = "fsgnn_external_baseline",
) -> Dict[str, Any]:
    if datasets is None:
    ...
    if not runnable_datasets:
        raise SystemExit("No runnable datasets found for FSGNN baseline.")

    runs_path = os.path.join(LOGS_DIR, f"fsgnn_runs_{output_tag}.jsonl")
    csv_path = os.path.join(REPORTS_DIR, f"fsgnn_results_{output_tag}.csv")
    ...
    print(f"\nCompleted {len(all_records)} FSGNN runs in {wall_time:.1f}s")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run standalone FSGNN external baseline.")
    parser.add_argument("--split-dir", default=None)
    parser.add_argument("--datasets", nargs="+", default=None)
    parser.add_argument("--splits", nargs="+", type=int, default=list(range(10)))
    parser.add_argument("--output-tag", default="fsgnn_external_baseline")
